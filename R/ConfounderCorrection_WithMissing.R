...
#'
#' @export
CC.Missing <- function(Y, X, Z=NULL, K, Miss.Mech, ind.samples=NULL, max.miss.perp=0.5, max.miss.image=0.5, BH.min=NULL, method = c("sIPW", "IPW"), include.updates=T, est.Beta=T) {
...
Estimation.IPW <- function(Y=Y, Cov=Cov, Weights, Var.Weights, max.miss.C=0.05, ind.int=NULL, ind.analyze) {
  ...
  if (length(ind.int) > 1) {
    out$f <- matrix(NA, nrow=p, ncol=length(ind.int))
